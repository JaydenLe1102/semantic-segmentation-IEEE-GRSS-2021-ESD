...
class SegmentationCNN(nn.Module):
    def __init__(self, in_channels: int, out_channels: int,  depth: int = 2, embedding_size: int = 64,
        ...
        for i, pool_size in enumerate(pool_sizes):
            ...
            out_channels = embedding_size if i == 0 else out_channels * 2
            self.encoders.append(
                Encoder(in_channels, out_channels, depth, kernel_size, pool_size)
            )

        self.decoder  = nn.Conv2d(out_channels, self.out_channels, kernel_size=1)
