...
class Subtile:
    ...
    def save(self, dir: str | os.PathLike):
        ...
        data = {key: value.astype(np.float32) for key, value in self.satellite_stack.items()}
        np.savez(
    ...
    def load(self, file_name: str | os.PathLike):
        ...
        data_dict = {key: value.astype(np.float32) for key, value in np.load(file_name).items()}
        self.satellite_stack = data_dict
...
def get_tile_ground_truth(
    ...
    end_x = start_x + size_gt[0]
    end_y = start_y + size_gt[1]
    ...
    if end_x > gt_parent.shape[2] or end_y > gt_parent.shape[3]:
        raise ValueError("The slice is out of bounds!!!")

    # return the slice of the ground truth parent image
    gt_child = gt_parent[:, :, start_x:end_x, start_y:end_y].copy()
...
def get_tile_satellite(
    ...
    start_x = scale_factor * size_gt[0] * x_sat
    end_x = scale_factor * size_gt[0] * (x_sat + 1)
    start_y = scale_factor * size_gt[1] * y_sat
    end_y = scale_factor * size_gt[1] * (y_sat + 1)

    sat_child = sat_parent[:, :, start_x:end_x, start_y:end_y]
...
def grid_slice(
    ...
    scale_factor = 50  # Assuming a fixed scale factor for simplicity
    tile_size_sat = tile_size_gt * scale_factor

    # Check if the ground truth stack is divisible by the tile size
    gt_parent = satellite_stack['gt']

    # Calculate the number of tiles needed in the x dimension
    num_tiles_x = gt_parent.shape[2] // tile_size_gt
    # Calculate the number of tiles needed in the y dimension
    num_tiles_y = gt_parent.shape[3] // tile_size_gt
    ...
    for x_gt in range(num_tiles_x):
        for y_gt in range(num_tiles_y):
            satellite_tiles = {}

            for satellite_type, stack in satellite_stack.items():
                if satellite_type == 'gt':
                    gt_slice = get_tile_ground_truth(gt_parent, x_gt, y_gt, tile_size_gt)
                    satellite_tiles[satellite_type] = gt_slice
                else:
                    sat_slice = get_tile_satellite(stack, x_gt, y_gt, tile_size_gt, scale_factor)
                    satellite_tiles[satellite_type] = sat_slice

            tile_metadata = metadata_to_tile_metadata(metadata_stack, x_gt, y_gt, tile_size_gt)
            tile_metadata.parent_tile_id = parent_tile_id

            subtile = Subtile(satellite_stack=satellite_tiles, tile_metadata=tile_metadata)
...
def restitch(
    ...
    file_path = dir / f"{tile_id}_{range_x[0]}_{range_y[0]}.npz"
    data = np.load(file_path)[f"{satellite_type}"]

    time, bands, width, height = data.shape
    ...
    for i in range(range_x[0], range_x[1]):
        tile_metadatas.append([])
        for j in range(range_y[0], range_y[1]):
            file_path = dir / f"{tile_id}_{i}_{j}.npz"
            subtile = Subtile()
            subtile.load(file_path)
            tile_metadatas[-1].append(subtile.tile_metadata)

            final_array[
                :,
                :,
                (i - range_x[0]) * width : (i - range_x[0] + 1) * width,
                (j - range_y[0]) * height : (j - range_y[0] + 1) * height,
            ] = subtile.satellite_stack[f"{satellite_type}"]

    return final_array, tile_metadatas
