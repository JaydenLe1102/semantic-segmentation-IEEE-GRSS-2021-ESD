...
def main():
    import pyprojroot
    root = pyprojroot.here()
    processed_dir = root / 'data'/'processed'/'Train1x1/subtiles'
    raw_dir = root / 'data'/'raw'/'Train'
    esd_dm = ESDDataModule(processed_dir, raw_dir, batch_size=1, selected_bands={"sentinel1": ["VV", "VH"], "viirs_maxproj": ["0"]}, tile_size_gt=1)
    esd_dm.prepare_data()
    esd_dm.setup("fit")
